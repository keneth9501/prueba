...
AND upper (categoria) IN ('RETORNABLES','PROMOCIÓN','JUEVES CERVECERO'))
...
AND upper (categoria) IN ('CERVEZA', 'RETORNABLES','PROMOCIÓN','JUEVES CERVECERO'))
...
where cast (fecha as date) = current_date - 1
...
worksheet2.add_table('A1:H700', {'data':dfdetalle,
...
worksheet1.add_table('A3:D600', {'data':df,
'style': 'Table Style Medium 2','header_row': True,
'columns': [{'header': 'PEDIDOID'},
            {'header': 'FECHA'},
            {'header': 'CATEGORIA'},
